...
// Official page link for a legislator. State members link to their
// palegis.us bio; federal members use an officialUrl field stored on the
// member record (senate.gov / house.gov). Returns null if no usable URL.
function officialPageUrl(m) {
  if (!m) return null;
  if (m.chamber === 'US' || m.chamber === 'UH') return m.officialUrl || null;
  // Derive palegis member id from the photo URL: .../members/300/<id>.jpg
  const photoMatch = m.photo && m.photo.match(/\/members\/\d+\/(\d+)\.jpg/);
  const palegisId = photoMatch ? photoMatch[1] : null;
  if (!palegisId) return 'https://www.palegis.us/find-my-legislator';
  if (m.chamber === 'S') return `https://www.palegis.us/senate/members/senate-member-bio?memberId=${palegisId}`;
  if (m.chamber === 'H') return `https://www.palegis.us/house/members/house-member-bio?memberId=${palegisId}`;
  return null;
}

function officialPageLabel(m) {
  if (!m) return '';
  if (m.chamber === 'US') return 'View on senate.gov';
  if (m.chamber === 'UH') return 'View on house.gov';
